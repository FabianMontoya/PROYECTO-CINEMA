...
public class Minioms extends JFrame implements ActionListener {

    JLabel I1, N1, D1, D2, D3, D4, D5;
    JButton Reservar;
    String Pelicula = "Minions", Usuario = "", LPeli = "Los Minions", IDPelicula="3";
    int IDUsuario=0;
    JMenu Menu1;
    JMenuBar MBarra;
    JMenuItem Volver, Salir;

    public Minioms(String User, int IDUser) {

        super("Los Minions");
        setLayout(null);
        Usuario = User;
        IDUsuario = IDUser;
        I1 = new JLabel(new ImageIcon(getClass().getResource("minions.jpg")));
        I1.setBounds(5, 15, 140, 200);
        I1.setVisible(true);

        String titulo = "<html><body><font color=#FFFF00> Los Minions</font></body></html>";
        N1 = new JLabel(titulo);
        N1.setBounds(50, 220, 200, 13);

        String texto = "<html><body> <font face=\"Comic Sans MS\">  <i> SINOPSIS:</i></font><p align=\"justify\">  La historia de Los Minions se remonta al principio de los tiempos. Empezaron siendo organismos amarillos unicelulares que evolucionaron a través del tiempo, poniéndose siempre al servicio de los amos más despreciables. Ante su incapacidad para mantener a esos amos – desde el T. Rex a Napoleón –, los Minions acaban encontrándose solos y caen en una profunda depresión. Sin embargo, uno de ellos, llamado Kevin, tiene un plan. Acompañado por el rebelde Stuart y el adorable Bob, emprende un emocionante viaje para conseguir una jefa a quien servir, la terrible Scarlet Overkill.  </p></body></html>";
        D1 = new JLabel(texto);
    ...
    public void control() {
        ...
        if (ax == JOptionPane.NO_OPTION) {
            Minioms c = new Minioms(Usuario, IDUsuario);
            c.setSize(500, 350);
